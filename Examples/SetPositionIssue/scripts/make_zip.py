...
def  main():
  ...
  installer = "\installer\SetPositionIssue Installer.exe"
   
  if demo:
    installer = "\installer\SetPositionIssue Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SetPositionIssue manual.pdf" 
  ]
  ...
  ZIPNAME = "SetPositionIssue-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SetPositionIssue-v" + FULLVERSIONSTR + "-win-demo.zip"
